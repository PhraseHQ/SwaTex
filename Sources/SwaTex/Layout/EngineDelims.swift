...
func nodeContainsMiddle(_ node: ParseNode, _ style: MathStyle) -> Bool {
    ...
    case .middle:
        return true
    case let .ordGroup(body, _), let .mclass(_, body, _):
        return body.contains { nodeContainsMiddle($0, style) }
    case let .supSub(base, sup, sub):
        return base.map { nodeContainsMiddle($0, style) } ?? false
            || sup.map { nodeContainsMiddle($0, style.superscript) } ?? false
            || sub.map { nodeContainsMiddle($0, style.subscript_) } ?? false
    case let .genfrac(_, numer, denom, _, _, _, _):
        return nodeContainsMiddle(numer, style.numerator)
            || nodeContainsMiddle(denom, style.denominator)
    case let .sqrt(body, index):
        return nodeContainsMiddle(body, style.cramped)
            || index.map { nodeContainsMiddle($0, style.superscript.superscript) } ?? false
    case let .accent(_, _, _, base), let .accentUnder(_, _, _, base):
        return nodeContainsMiddle(base, style)
    case let .op(_, _, _, _, _, _, body):
        return body?.contains { nodeContainsMiddle($0, style) } ?? false
    case .leftRight:
        return false
    case let .operatorName(body, _, _, _):
        return body.contains { nodeContainsMiddle($0, style) }
    case let .font(_, body):
        return nodeContainsMiddle(body, style)
    case let .text(body, _):
        return body.contains { nodeContainsMiddle($0, style.textEquivalent) }
    case let .sizing(_, body):
        return body.contains { nodeContainsMiddle($0, style.textEquivalent) }
    case let .color(_, body):
        return body.contains { nodeContainsMiddle($0, style) }
    case let .styling(nodeStyle, body):
        let newStyle = styleStrToMathStyle(nodeStyle)
        return body.contains { nodeContainsMiddle($0, newStyle) }
    case let .overline(body), let .underline(body):
        return nodeContainsMiddle(body, style)
    case let .phantom(body):
        return body.contains { nodeContainsMiddle($0, style) }
    case let .vphantom(body):
        return nodeContainsMiddle(body, style)
    case let .smash(body, _, _):
        return nodeContainsMiddle(body, style)
    case let .array(info):
        return info.body.contains { row in row.contains { nodeContainsMiddle($0, style) } }
    case let .enclose(_, _, _, body):
        return nodeContainsMiddle(body, style)
    case let .lap(_, body):
        return nodeContainsMiddle(body, style)
    case let .raiseBox(_, body):
        return nodeContainsMiddle(body, style)
    ...
    case let .xArrow(_, body, below):
        return nodeContainsMiddle(body, style.superscript)
            || below.map { nodeContainsMiddle($0, style.subscript_) } ?? false
    case let .cdArrow(_, labelAbove, labelBelow):
        return labelAbove.map { nodeContainsMiddle($0, style.superscript) } ?? false
            || labelBelow.map { nodeContainsMiddle($0, style.subscript_) } ?? false
    case let .mathChoice(display, text, script, scriptscript):
        ...
        return branch.contains { nodeContainsMiddle($0, style) }
    case let .horizBrace(_, _, base):
        return nodeContainsMiddle(base, style)
    ...
    case let .htmlMathML(html, _):
        return html.contains { nodeContainsMiddle($0, style) }
    default:
        return false
    }
}
...
func layoutLeftRight(
    ...
    if bodyContainsMiddle(body, options.style) {
        ...
        let innerFirst = layoutExpression(body, optsFirst, isRealGroup: true)
        totalHeight = leftRightDelimTotalHeight(innerFirst, options)
        // Second pass: layout with totalHeight so \middle stretches to match \left and \right.
        var optsSecond = options
        optsSecond.leftrightDelimHeight = totalHeight
        inner = layoutExpression(body, optsSecond, isRealGroup: true)
    } else {
...
func scaleSvgPathToEm(_ cmds: [PathCommand]) -> [PathCommand] {
    let s = 0.001
    return cmds.map { c in
        switch c {
        case let .moveTo(x, y): .moveTo(x: x * s, y: y * s)
        case let .lineTo(x, y): .lineTo(x: x * s, y: y * s)
        case let .cubicTo(x1, y1, x2, y2, x, y):
            .cubicTo(x1: x1 * s, y1: y1 * s, x2: x2 * s, y2: y2 * s, x: x * s, y: y * s)
        case let .quadTo(x1, y1, x, y): .quadTo(x1: x1 * s, y1: y1 * s, x: x * s, y: y * s)
        case .close: .close
        }
    }
}
...
func makeShortmidBox(_ options: LayoutOptions) -> LayoutBox {
    ...
    let scaled = scaleSvgPathToEm(raw)
...
func makeStretchyDelim(
    ...
    let vertNaturalHeight = 1.0  // MainRegular |: 0.75+0.25
